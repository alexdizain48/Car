...
import com.alex.car.Helper.CarLab;
import com.alex.car.adapter.AdapterDtp;
...
public class InfoCar extends AppCompatActivity {
    ...
    ConstraintLayout constrLayoutTehHaract;
    CardView btnOpenTehnHarakter, btnOpenProbeg, btnOpenTehOsmotr, btnOpenDtp;

    RecyclerView recycleTehOsmotr, recycleDtp;
    RecyclerView.LayoutManager tehOsmLM, dtpLM;

    AdapterTehOsmotr mAdapterTehOsmotr;
    AdapterDtp mAdapterDtp;
    ...
    Button buy;
    TextView modelAuto, textHeadProbeg, textHeadTehOsmotr, textHeadDtp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // if (!isConnected(InfoCar.this)) buildDialog(InfoCar.this).show();
        //else {
            ...
            //modelAuto = (TextView)findViewById(R.id.modelAuto);

            constrLayoutTehHaract = (ConstraintLayout) findViewById(R.id.constrLayoutTehHaract);
           ...
           /* CarLab carlab = CarLab.get(getApplicationContext());
            items = carlab.getMainModels();*/
    ...
    private void setDataDtp() {

        mDtpList = new ArrayList<>();
        mDtpList.add(new Dtp("20.12.12", "Ужас", "Липецк"));

        recycleDtp = (RecyclerView)findViewById(R.id.recycleDtp);
        recycleDtp.setHasFixedSize(true);
        dtpLM = new LinearLayoutManager(this);
        recycleDtp.setLayoutManager(dtpLM);

        mAdapterDtp = new AdapterDtp(mDtpList, this);
        recycleDtp.setAdapter(mAdapterDtp);

        DividerItemDecoration dividerTehOsmotrItem = new DividerItemDecoration(recycleDtp.getContext(), LinearLayoutManager.VERTICAL);
        Drawable drawable = ContextCompat.getDrawable(getBaseContext(), R.drawable.divider);
        if(drawable!=null)
            dividerTehOsmotrItem.setDrawable(drawable);
        recycleDtp.addItemDecoration(dividerTehOsmotrItem);
    }
